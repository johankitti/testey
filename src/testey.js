...
const scanForTests = currPath => {
  try {
    ...
    if (
      ...
      const pathNameToTest = `${currPath
        .split(`/${testFileName}`)
        .join("")}/__tests__/${testFileName.split(".")[0]}.spec.js`;
      try {
...
paths
  ...
  .forEach(path => {
    ...
    const fileCoveragePercentage = (
      (filesWithTests.length / totalFiles) *
      100
    ).toFixed(1);
    console.log("\x1b[36m%s\x1b[0m", `=== Status for ${path} ===`);
    console.log(
      `Files with tests ${
        filesWithTests.length
      }/${totalFiles} (${fileCoveragePercentage}%)`
    );
  });
...
const singleFilesWithoutTests =
  singleFilePaths
    ...
    .reduce((agg, result) => agg + result.filesWithoutTests.length, 0) > 0;
